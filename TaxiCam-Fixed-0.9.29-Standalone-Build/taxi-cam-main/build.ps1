...
$windresCommand = Get-Command llvm-windres.exe -ErrorAction SilentlyContinue
...
$compiler = $clangCommand.Source
$windres = $windresCommand.Source
if ($Bootstrap) {
    Write-Warning '-Bootstrap is retained for compatibility but is no longer required; the local PATH toolchain is used.'
}
